...
# Setup dns
while [ -z "$(kubectl get svc -n scf tcp-router-public --output='jsonpath={.status.loadBalancer.ingress}')" ]; do
  sleep 5
done
...
while [ -z "$(kubectl get svc -n scf router-public --output='jsonpath={.status.loadBalancer.ingress}')" ]; do
  sleep 5
done
...
while [ -z "$(kubectl get svc -n scf "${ssh_proxy_svc}" --output='jsonpath={.status.loadBalancer.ingress}')" ]; do
  sleep 5
done
